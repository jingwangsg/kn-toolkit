...
class BoxList(object):
    ...
    def convert(self, mode):
        ...
        Args:
            mode : xyxy xywh
        """
        if mode not in ("xyxy", "cxcywh", "xywh"):
            raise ValueError("mode should be 'xyxy' or 'cxcywh'")
        if mode == self.mode:
            return self
        ...
        if mode == "xyxy":
            ...
            bbox = torch.cat(((xmin + xmax) / 2, (ymin + ymax) / 2, (xmax - xmin), (ymax - ymin)), dim=-1)
            bbox = BoxList(bbox, self.size, mode=mode)
        elif mode == "xywh":
    ...
    def _split_into_xyxy(self):
        if self.mode == "xyxy":
            ...
            return (xc - 0.5 * w, yc - 0.5 * h, xc + 0.5 * w, yc + 0.5 * h)
        elif self.mode == "xywh":
            x, y, w, h = self.bbox.split(1, dim=-1)
            return (x, y, w + x, h + y)
        else:
    ...
    def shift(self, padded_size, left: int, top: int):
        ...
        shifted_box = torch.cat((shifted_xmin, shifted_ymin, shifted_xmax, shifted_ymax), dim=-1)
    ...
    def normalize(self):
        ...
        normalized_bbox = torch.cat((xmin, ymin, xmax, ymax), dim=-1)
        bbox = BoxList(normalized_bbox, self.size, mode="xyxy")
        return bbox.convert("cxcywh")
