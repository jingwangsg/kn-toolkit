...
def prepare_for_dataloader(datapipe, batch_size, is_train=True):
    # if dist.is_initialized() and dist.is_available():
    #     world_size = int(os.environ["WORLD_SIZE"])
    # else:
    #     world_size = 1
    if is_train and not global_get(Signal.train_no_shuffle, False):
        datapipe = datapipe.shuffle()
        # datapipe = pad_to_multiple_of(datapipe, world_size * batch_size)
