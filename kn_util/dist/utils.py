...
# https://github.dev/open-mmlab/mmengine/dist/utils.py
...
from .device_utils import is_mlu_available, is_npu_available, is_musa_available
...
def get_local_group() -> Optional[ProcessGroup]:
    ...
    if _LOCAL_PROCESS_GROUP is None:
        raise RuntimeError("Local process group is not created, please use " "`init_local_group` to setup local process group.")
...
def _init_dist_pytorch(backend, init_backend="torch", **kwargs) -> None:
    ...
    if is_mlu_available():
        ...
        if init_backend == "torch":
            ...
            raise ValueError('supported "init_backend" is "torch" or "deepspeed", ' f"but got {init_backend}")
    ...
    if isinstance(data, Tensor):
        ...
        for v in data.values():
            ...
            if pre is None:
                ...
                if cur != pre:
                    raise ValueError("device type in data should be consistent, but got " f"{cur} and {pre}")
        if pre is None:
        ...
        raise TypeError("data should be a Tensor, sequence of tensor or dict, " f"but got {data}")
...
def cast_data_device(
    data: Union[Tensor, Mapping, Iterable], device: torch.device, out: Optional[Union[Tensor, Mapping, Iterable]] = None
    ...
    if out is not None:
        if type(data) != type(out):
            raise TypeError("out should be the same type with data, but got data is " f"{type(data)} and out is {type(data)}")
    ...
    if isinstance(data, Tensor):
        ...
        if out is not None:
            ...
            if data_len != out_len:
                raise ValueError("length of data and out should be same, " f"but got {data_len} and {out_len}")
        ...
        raise TypeError("data should be a Tensor, list of tensor or dict, " f"but got {data}")
