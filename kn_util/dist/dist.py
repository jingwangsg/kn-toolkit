...
import os
from ..utils.version import digit_version, TORCH_VERSION
...
def _get_reduce_op(name: str) -> torch_dist.ReduceOp:
    ...
    if name.lower() not in op_mappings:
        raise ValueError(f"reduce op should be one of {op_mappings.keys()}, bug got {name}")

    return op_mappings[name.lower()]


def all_reduce(data: Tensor, op: str = "sum", group: Optional[ProcessGroup] = None) -> None:
...
def gather(data: Tensor, dst: int = 0, group: Optional[ProcessGroup] = None) -> List[Optional[Tensor]]:
...
def _broadcast_object_list(object_list: List[Any], src: int = 0, group: Optional[ProcessGroup] = None) -> None:
...
def broadcast_object_list(data: List[Any], src: int = 0, group: Optional[ProcessGroup] = None) -> None:
    ...
    if get_world_size(group) > 1:
        ...
        if digit_version(TORCH_VERSION) >= digit_version("1.8.0") and not is_npu_available():
            torch_dist.broadcast_object_list(data, src, group)
        else:
            _broadcast_object_list(data, src, group)


def all_reduce_dict(data: Dict[str, Tensor], op: str = "sum", group: Optional[ProcessGroup] = None) -> None:
    ...
    if world_size > 1:
        ...
        split_tensors = [x.reshape(shape) for x, shape in zip(torch.split(flatten_tensor, tensor_sizes), tensor_shapes)]

        for k, v in zip(keys, split_tensors):
            data[k] = v


def _all_gather_object(object_list: List[Any], obj: Any, group: Optional[ProcessGroup] = None) -> None:
...
def _validate_output_list_for_rank(my_rank: int, dst: int, gather_list: Optional[list]) -> None:
...
def _gather_object(obj: Any, object_gather_list=None, dst: int = 0, group: Optional[ProcessGroup] = None) -> None:
...
def gather_object(data: Any, dst: int = 0, group: Optional[ProcessGroup] = None) -> Optional[List[Any]]:
    """Gathers picklable objects from the whole group in a single process.
    Similar to :func:`gather`, but Python objects can be passed in. Note that
    the object must be picklable in order to be gathered.
...
def _all_reduce_coalesced(
    tensors: List[torch.Tensor], bucket_size_mb: int = -1, op: str = "sum", group: Optional[ProcessGroup] = None
) -> None:
...
def all_reduce_params(
    params: Union[List, Generator[torch.Tensor, None, None]],
    coalesce: bool = True,
